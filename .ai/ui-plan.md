...
## 7. Analiza punktów bólu i rozwiązań UI

| Punkt bólu | Rozwiązanie UI |
| :--- | :--- |
| Zapominanie o szczegółach u weterynarza. | Szybki dostęp do historii i wyszukiwanie (przyszłość) na profilu. |
| Zbyt długi czas dodawania wpisu. | Formularz zredukowany do 3 kroków, duże kafelki kategorii, domyślna data. |
| Trudność w nawigacji na telefonie. | Mobile-first, duże touch-targety (44px), skracane breadcrumbsy. |
| Frustracja przy błędach formularza. | Walidacja w czasie rzeczywistym (Zod) i jasne komunikaty `aria-live`. |
| Brak poczucia bezpieczeństwa danych. | Profesjonalne strony 403/404, izolacja sesji w middleware. |

## 8. Mapowanie historyjek użytkownika (US)

-   **US-001 - US-003:** Obsłużone przez widoki `/login`, `/register` i Header.
-   **US-004 - US-006:** Obsłużone przez `/dashboard` i `/pets/new`.
-   **US-007 - US-009:** Obsłużone przez `/pets/[petId]` i `/pets/[petId]/entries/new`.
-   **US-010:** Responsywność zaimplementowana w każdym komponencie (Tailwind).
-   **US-012 - US-013:** Obsłużone przez logikę `DatePicker` i opcjonalność pola `note`.
